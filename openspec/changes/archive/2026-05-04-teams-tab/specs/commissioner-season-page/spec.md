...
The page SHALL render three tabs — Roster, Teams, Schedule — in that order. Each tab label SHALL display a checkmark indicator when the corresponding requirement `isMet` is `true`, and a warning indicator when `isMet` is `false`. The Roster tab body SHALL render the live roster table (see `roster-tab` capability). The Teams tab body SHALL render the live teams management UI (see `teams-tab` capability). The Schedule tab body remains an empty placeholder until its implementation proposal is applied.
...
#### Scenario: Schedule tab body is a placeholder
